...
function build3DPreviewWindow()
    ...
    -- The alert column is drawn for every row below 100%: a quiet "Minor"
    -- chip for Healthy-band damage (pct >= g2y), an amber "Worn" chip for
    -- the yellow band, and a red "Crit" chip for the red band. Rows at
    -- 100% leave the column blank so a pristine set reads as a clean
    -- right edge. The tri-state matches the "Needs repair / Worn /
    -- Critical" headline so the column can never be empty while the
    -- headline complains about damage.
    --
        ...
        for i = 1, MAX_ROWS do
            ...
            if d then
                ...
                elseif d.pct < g2y then
                    ...
                    w.alert:Show()
                elseif d.pct < 100 then
                    -- v1.0.1: Minor chip covers the gap between g2y (e.g.
                    -- 75%) and 100%. Before this tier the Alert column
                    -- stayed completely blank whenever every damaged item
                    -- was still in the Healthy band, which contradicted
                    -- the "Needs repair" headline and read as a broken
                    -- column. The chip is intentionally quiet — dim
                    -- brown-bronze fill, muted gold border, soft gold
                    -- middot + "Minor" — so it reports "below full, not
                    -- urgent" without competing with real Worn / Crit
                    -- warnings. "·" (U+00B7) is one UTF-8 byte sequence
                    -- \194\183; same encoding pattern used elsewhere
                    -- in this file for the dim suffix separator.
                    w.alertBg:SetVertexColor(0.14, 0.11, 0.06, 1.00)
                    w.alertEdge:SetVertexColor(0.70, 0.60, 0.32, 1.00)
                    w.alertLbl:SetText("\194\183 Minor")
                    w.alertLbl:SetTextColor(0.95, 0.82, 0.48)
                    w.alert:Show()
                else
                    w.alert:Hide()
                end
...
f:SetScript("OnEvent", function(_, event, ...)
    if event == "PLAYER_LOGIN" then
        ...
        print("|cffffd200SlotStatus|r v" .. "1.0.1" .. " loaded \226\128\148 |cffffffff/ss options|r for the panel.")
